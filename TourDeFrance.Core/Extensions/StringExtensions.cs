...
namespace TourDeFrance.Core.Extensions
{
	...
	public static class StringUtil
	{
		...
		public static string ReplaceAll(this string source, params string[] replacements)
		{
			...
			{
				throw new ArgumentException("length of string replacements must be even", nameof(replacements));
			}
		...
		public static string EscapeString(this string text)
		{
			...
			var result = new StringBuilder(2*text.Length);
		...
		public static string ReplaceWithinDelimiters(this string text, string startDelimiter, string endDelimiter,
			string replace, StringComparison comparison)
		{
		...
		/// <returns>String with delimited text replaced.</returns>
		public static string ReplaceWithinDelimiters(this string text, string startDelimiter, string endDelimiter,
			Converter<string, string> callback, StringComparison comparison)
		{
			...
			switch (comparison)
			{
				...
				case StringComparison.OrdinalIgnoreCase:
					...
					break;
				default:
					throw new ArgumentException("unknown comparison type", nameof(comparison));
			}
			...
			else
			{
				while (true)
				{
					...
					buffer.Append(
						callback(text.Substring(foundStart + startDelimiter.Length, foundEnd - (foundStart + startDelimiter.Length))));
		...
		///     <see langword="True" /> if case should not be considered in comparison.
		/// </param>
		/// <returns>
		///     <see langword="True" /> if left- and right-hand sides are equal.
		/// </returns>
		...
		/// </param>
		/// <returns>
		///     <see langword="True" /> if value matches the beginning of the input string; otherwise, <see langword="False" />.
		/// </returns>
		...
		/// </param>
		/// <returns>
		///     <see langword="True" /> if value matches the end of the input string; otherwise, <see langword="False" />.
		/// </returns>
		...
		/// </param>
		/// <returns>
		///     The zero-based index position of value if that string is found, or -1 if it is not. If value is
		///     <see
		///         cref="string.Empty" />
		...
		/// </param>
		/// <returns>
		///     The index position of the value parameter if that string is found, or -1 if it is not. If value is
		///     <see
		///         cref="string.Empty" />
		...
		/// <param name="value">The System.String to find.</param>
		/// <returns>
		///     <see langword="True" /> if value is found in the input string; otherwise, <see langword="False" />.
		/// </returns>
		...
		public static string CreateAlphaNumericKey(this int length)
		{
			if (length <= 0)
			{
			...
			while (index < result.Length)
			{
				...
				for (int i = 0; (i < buffer.Length) && (index < result.Length); ++i)
				{
					...
					if (buffer[i] <= AlphanumChars.Length)
					{
						result[index++] = AlphanumChars[buffer[i] - 1];
					}
				}
			}
		...
		/// <returns>MD5 hash.</returns>
		public static byte[] ComputeHash(this string text, Encoding encoding)
		{
		...
		/// <param name="encoding">Encoding to use to get input string bytes.</param>
		/// <returns>MD5 hash string.</returns>
		public static string ComputeHashString(this string text, Encoding encoding)
		{
		...
		public static byte[] BytesFromHexString(this string text)
		{
			if ((text == null) || (text.Length%2 != 0))
			{
				throw new ArgumentException("invalid length for a hex string", nameof(text));
			}
			...
			for (int i = 0; i < text.Length; i += 2)
			{
				...
				{
					throw new ArgumentException("hex string contains invalid value", nameof(text));
				}
		...
		public static char IntToHexChar(this int n)
		{
		...
		public static string CutIfLongerThan(this string value, int length, bool includeEllipsis = true)
		{
			if (value == null)
				return null;
			if (value.Length > length)
			{
				return includeEllipsis ? value.Substring(0, length - 3) + "..." : value.Substring(0, length);
			}
			return value;
		}
		...
		public static bool? AsBool(this string value)
		{
			...
			return value == "1" || value == "y" || value == "true" || value == "yes";
		}
